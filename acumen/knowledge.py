...
def _is_recent(item, max_age_days, now):
    ...
    if researched is None or now is None:
        return False
    return -300 <= (now - researched).total_seconds() <= max_age_days * 86400
...
def merge_candidate(existing, incoming):
    ...
    # Saving a snapshot or recording usage must never renew its research date.
...
def select_answer(items, query, *, max_age_days=30, now=None):
    """Reuse recent, unambiguous answers; repetition is not new evidence."""
